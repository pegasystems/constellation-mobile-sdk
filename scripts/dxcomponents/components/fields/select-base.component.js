...
import {
    ...
    initializeListSourceConfig,
    populateItems,
    preProcessColumns
} from "./select_utils.js";
...
export class SelectBaseComponent extends PicklistBaseComponent {
    ...
    updateSelf() {
        ...
        const isReferenceField =
            !!configProps.referenceType && ["case", "data"].includes(configProps.referenceType.toLowerCase());

        [listType, columns, datasource, parameters] = initializeListSourceConfig(
    ...
    updateOptions(fieldMetadata, className, deferDatasource, isSourceDataPage, datasource) {
        ...
        const localeContext =
            metaData?.datasource?.tableType === "DataPage" ? Constants.DATAPAGE : Constants.ASSOCIATED;

        const options =
            buildListSourceItems(deferDatasource, isSourceDataPage, this.listItems, this.pConn, datasource) ?? [];
