...
function updatePageListPropertyValue(value) {
    value =
        value.substring(0, value.indexOf(SQUARE_BRACKET_START)) +
        value.substring(value.indexOf(SQUARE_BRACKET_END) + 1);
    return value;
}
...
export const populateItems = (response, displayFieldMeta, dataApiObj) => {
...
export const buildListSourceItems = (deferDatasource, isSourceDatapage, listItems, pConnect, datasource) => {
    let listSourceItems = [];
