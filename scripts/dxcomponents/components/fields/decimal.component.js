...
export class DecimalComponent {
  ...
  updateSelf() {
    ...
    if (configProps.decimalPrecision) {
      const decimalPrecision = configProps.decimalPrecision
      if (typeof decimalPrecision === 'string') {
        this.props.decimalPrecision = parseInt(decimalPrecision, 10);
      } else {
        this.props.decimalPrecision = decimalPrecision;
      }
    }
