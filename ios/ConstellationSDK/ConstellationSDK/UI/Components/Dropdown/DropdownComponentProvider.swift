...
class DropdownComponentProvider: ComponentProvider {
    ...
    required init() {
        ...
        view = AnyView(DropdownView(properties: properties))
    ...
    public func updateProperties(_ jsonInput: String) throws {
        try JSONDecoder()
            .decode(DecodableDropdownProps.self, from: Data(jsonInput.utf8))
            .apply(to: properties)
    }
    
}

fileprivate struct DecodableDropdownProps: Decodable {
    let value: String
    let label: String
    ...
    func apply(to observableProps: DropdownProps) {
