...
struct CurrencyView: View {
        ...
        VStack(alignment: .leading, spacing: 5) {
            ...
            HStack {
                ...
                .onChange(of: properties.value) { newValue in
                    properties.value = formatCurrency(value: newValue, decimalPlaces: properties.decimalPrecision)
                }
    ...
    private func formatCurrency(value: String, decimalPlaces: Int) -> String {
        let filtered = value.filter { "0123456789,.".contains($0) }.replacingOccurrences(of: ",", with: ".")
        if let decimalIndex = filtered.firstIndex(of: ".") {
            let beforeDecimal = filtered[..<decimalIndex]
            let afterDecimal = filtered[decimalIndex...].prefix(decimalPlaces + 1)
