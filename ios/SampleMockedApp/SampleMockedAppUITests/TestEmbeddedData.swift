...
final class TestEmbeddedData: MockedAppTestCase {
    func testEmbeddedData() {
        ...
        verifyDisplayOnlyValue(at: 3, "Bayerische Motoren Werke")
        verifyDisplayOnlyValue(at: 4, "Vision Neue Klasse")
        ...
        verifyDisplayOnlyValue(at: 0, "Audi")
        verifyDisplayOnlyValue(at: 1, "A5")
    }

    private func verifyDisplayOnlyValue(at index: Int, _ expected: String) {
        let field = app.textFields.element(boundBy: index)
        XCTAssertEqual(expected, field.value as? String)
